...
def plot_contour_paths(X_full, y_full, histories, labels, initial_w, true_w, title, fig_size=(10, 7), dpi=100):
    ...
    colors = plt.cm.viridis(np.linspace(0, 0.8, n_paths))  
    ...
    ax.plot(initial_w[0,0], initial_w[1,0], 'go', markersize=12, label='Старт')  
    if true_w is not None:
        ax.plot(true_w[0,0], true_w[1,0], 'm*', markersize=14, label='Истинный минимум')  
    
    ax.set_xlabel("Вес w1", fontsize=12)  
    ax.set_ylabel("Вес w2", fontsize=12)  
